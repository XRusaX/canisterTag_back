...
//		return getKey(data., data.CAN_VOLUME_ML, data.CAN_MANUFACTURER_NAME, data.CAN_DESINFICTANT_NAME,
//				data.CAN_ACTIVE_INGRIDIENT_NAME, data.CAN_INGRIDIENT_CONCENTRATION);
////		return getKey(data.rfidData);
//	}
//
...
//	public static byte[] getKey(List<DataItem> dataStruct) throws UnsupportedEncodingException {
//		String key = "";
//		for (DataItem item : dataStruct) {
//			if (item.tag == 39 || item.tag == 4 || item.tag == 5 || item.tag == 10 || item.tag == 8 || item.tag == 38) {
//				key += item.value;
//			}
//		}
